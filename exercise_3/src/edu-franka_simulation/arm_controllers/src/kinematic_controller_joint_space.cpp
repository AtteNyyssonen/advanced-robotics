#include "arm_controllers/kinematic_controller_joint_space.hpp"
...
controller_interface::CallbackReturn KinematicController::on_init()
{
...
controller_interface::InterfaceConfiguration KinematicController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration KinematicController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration cfg;
  cfg.type = controller_interface::interface_configuration_type::INDIVIDUAL;
  for (const auto & j : joints_) {
...
controller_interface::CallbackReturn KinematicController::on_configure(const rclcpp_lifecycle::State &)
{
  RCLCPP_INFO(get_node()->get_logger(), "Configuring KinematicController");
  ...
  if (kp_gains_.size() != joints_.size()) {
    kp_gains_.assign(joints_.size(), 5.0);
  }
...
controller_interface::CallbackReturn KinematicController::on_activate(const rclcpp_lifecycle::State &)
{
...
controller_interface::CallbackReturn KinematicController::on_deactivate(const rclcpp_lifecycle::State &)
{
...
controller_interface::return_type KinematicController::update(const rclcpp::Time &, const rclcpp::Duration & period)
{
  ...
  if (goal_active_) {
    ...
    double alpha = 0.5 * (1 - cos(M_PI * elapsed_time_ / traj_duration_));
    double alpha_dot = 0.5 * M_PI / traj_duration_ * sin(M_PI * elapsed_time_ / traj_duration_);

    KDL::Vector start = ee_start_.p;
    KDL::Vector goal  = ee_goal_.p;
    KDL::Vector x_des = start + alpha * (goal - start);
    KDL::Vector xdot_d = alpha_dot * (goal - start);

    KDL::Frame ee_now;
    KDL::ChainFkSolverPos_recursive fk_solver(kdl_chain_);
    fk_solver.JntToCart(kdl_q_, ee_now);

    KDL::Vector e_x = x_des - ee_now.p;
    ...
    Eigen::VectorXd dx(6);
    dx << xdot_d.x() + 1.0*e_x.x(), xdot_d.y() + 1.0*e_x.y(), xdot_d.z() + 1.0*e_x.z(), 0,0,0;

    Eigen::VectorXd qdot_d = J_pinv * dx;

    for (size_t i=0;i<n;++i) {
      desired_vel_vec[i] = qdot_d(i);
      double kp = (i < kp_gains_.size()) ? kp_gains_[i] : 1.0;
      out_vec[i] = qdot_d(i) + kp * (0.0);
      err_vec[i] = qdot_d(i) - kdl_qdot_(i);
    }

    if (elapsed_time_ / traj_duration_ >= 1.0) {
      goal_active_ = false;
      RCLCPP_INFO(get_node()->get_logger(), "Cartesian goal maybe reached");
    }
  }
...
bool KinematicController::init_kdl_from_urdf(const std::string & urdf_param)
{
  std::string urdf_xml;
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::KinematicController, 
